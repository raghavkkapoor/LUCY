...
import threading
import traceback
print("LUCY_TEST_PRINT_2026_08_22: UPDATED MAIN.PY LOADED")
...
def main():
    ...
    try:
        subprocess.run(
            ["powershell", "-ExecutionPolicy", "Bypass", "-File", BROWSER_LAUNCHER_SCRIPT_PATH],
        ...
        while True:
            try:
                ...
                if is_target_closed_error(error):
                    ...
                    request = f"This is a continuation of a previous goal/request by the user. User's original request was: {temp_request} and here's the latest output from Lucy trying to solve it: {incomplete_context}. Last script she tried executing was:\n{last_command_received}"
...
# Reduce powershell overhead by running a lucy's instance of it (powershell.exe) and run commands in it. This keeps persistent state across all commands in that session (Update system prompt for this)
